...
-- 武器追踪配置（新增）
local WeaponTrackConfig = {
    Enabled = false,
    TrackRange = 60,
    TrackCircleColor = Color3.new(1,1,0),
    TrackCircleRadius = 180,
}
local TrackCircleGui = nil
...
local function SaveAllConfig()
    local config = {
        ...
        PlayerWhitelist = PlayerLockList,
        WeaponTrack = WeaponTrackConfig
    }
...
local function LoadAllConfig()
    ...
    if success then
        ...
        if config.PlayerWhitelist then
            PlayerLockList = config.PlayerWhitelist
        end
        if config.WeaponTrack then
            for k, v in pairs(config.WeaponTrack) do WeaponTrackConfig[k] = v end
        end
    end
end
...
-- 武器追踪功能（新增）
local function UpdateTrackCircle()
    if not WeaponTrackConfig.Enabled or not TrackedPlayer then
        if TrackCircleGui then 
            TrackCircleGui:Destroy() 
            TrackCircleGui = nil 
        end
        return
    end
    if not TrackCircleGui then
        TrackCircleGui = Instance.new("ScreenGui")
        TrackCircleGui.Name = "TrackCircle"
        TrackCircleGui.Parent = CoreGui
        TrackCircleGui.IgnoreGuiInset = true
        TrackCircleGui.ResetOnSpawn = false

        local circle = Instance.new("Frame")
        circle.Name = "Circle"
        circle.Size = UDim2.new(0, WeaponTrackConfig.TrackCircleRadius*2, 0, WeaponTrackConfig.TrackCircleRadius*2)
        circle.Position = UDim2.new(0.5, -WeaponTrackConfig.TrackCircleRadius, 0.5, -WeaponTrackConfig.TrackCircleRadius)
        circle.BackgroundTransparency = 1
        circle.Parent = TrackCircleGui
        local stroke = Instance.new("UIStroke")
        stroke.Color = WeaponTrackConfig.TrackCircleColor
        stroke.Thickness = 2
        ...
        nameLabel.Position = UDim2.new(0.5,-100,0,0.1)
        nameLabel.BackgroundTransparency = 1
        nameLabel.TextColor3 = WeaponTrackConfig.TrackCircleColor
        nameLabel.TextStrokeTransparency = 0.5
        nameLabel.TextSize = 16
        nameLabel.Parent = TrackCircleGui
    end
    TrackCircleGui.TrackName.Text = "追踪: "..(TrackedPlayer and TrackedPlayer.Name or "无")
end

local function StartWeaponTrack()
    WeaponTrackConfig.Enabled = true
    TrackedPlayer = nil
    local trackLoop = RunService.Heartbeat:Connect(function()
        if not WeaponTrackConfig.Enabled then return end
        pcall(function()
            ...
            for _, p in pairs(Players:GetPlayers()) do
                ...
                -- 使用自瞄白名单逻辑
                if not PlayerLockList[p.UserId] then continue end
                local tChar = p.Character
                local tRoot = tChar and tChar:FindFirstChild("HumanoidRootPart")
                if not tRoot then continue end
                local dist = (myRoot.Position - tRoot.Position).Magnitude
                if dist < WeaponTrackConfig.TrackRange and dist < minDist then
                    minDist = dist
                    bestTarget = p
                end
            end
            TrackedPlayer = bestTarget
            UpdateTrackCircle()

            if TrackedPlayer then
                local tChar = TrackedPlayer.Character
                local tRoot = tChar and tChar:FindFirstChild("HumanoidRootPart")
                local tool = myChar and myChar:FindFirstChildOfClass("Tool")
                if tool and tRoot then
                    local handle = tool:FindFirstChild("Handle")
                    if handle then
                        handle.CFrame = CFrame.new(handle.Position, tRoot.Position)
                    end
                end
            end
        end)
...
local function StopWeaponTrack()
    WeaponTrackConfig.Enabled = false
    TrackedPlayer = nil
    UpdateTrackCircle()
end
...
local function UpdateRangedCrosshair()
    ...
    if not RangedCrosshairGui then
        ...
        cross.Size = UDim2.new(0, RangedWeaponConfig.CrosshairSize, 0, RangedWeaponConfig.CrosshairSize)
        cross.Position = UDim2.new(0.5, -RangedWeaponConfig.CrosshairSize/2, 0.5, -RangedWeaponConfig.CrosshairSize/2)
        cross.BackgroundColor3 = RangedWeaponConfig.CrosshairColor
        cross.Parent = RangedCrosshairGui
        local corner = Instance.new("UICorner")
        corner.CornerRadius = UDim.new(1, 0)
        corner.Parent = cross
    end
...
local function StartRangedWeaponOptimize()
    ...
    local aimLoop = RunService.Heartbeat:Connect(function()
        ...
        pcall(function()
            ...
            local ray = cam:ScreenPointToRay(center.X, center.Y)
            handle.CFrame = CFrame.new(handle.Position, ray.Origin + ray.Direction * 300)
        end)
    end)
    AddLoop(aimLoop)
end
...
-- 3. 替换原有OpenPlayerTeleportWindow 重做UI+连传修复+距离设置
local function OpenPlayerTeleportWindow()
    ...
    distLabel.Text = "距离:"..followDist
    distLabel.TextColor3 = Config.TextWhite
    distLabel.TextSize = 14
    distLabel.Parent = top

    local decBtn = Instance.new("TextButton")
    decBtn.Size = UDim2.new(0.08,0,0,30)
    decBtn.Position = UDim2.new(0.7,5,0,5)
    decBtn.BackgroundColor3 = Config.BtnBlue
    decBtn.Text = "-"
    decBtn.TextColor3 = Config.TextWhite
    decBtn.Parent = top

    local incBtn = Instance.new("TextButton")
    incBtn.Size = UDim2.new(0.08,0,0,30)
    incBtn.Position = UDim2.new(0.78,5,0,5)
    incBtn.BackgroundColor3 = Config.BtnBlue
    incBtn.Text = "+"
    incBtn.TextColor3 = Config.TextWhite
    incBtn.Parent = top
    ...
    stopFollowBtn.Size = UDim2.new(0,100,0,40)
    stopFollowBtn.Position = UDim2.new(0.02,0,0.02,0)
    stopFollowBtn.BackgroundColor3 = Config.BtnRed
    stopFollowBtn.Text = "退出连传"
    stopFollowBtn.TextColor3 = Config.TextWhite
    stopFollowBtn.Visible = false
    stopFollowBtn.Parent = sg

    local function StopLoopTp()
        if loopTp then loopTp:Disconnect() loopTp=nil end
        stopFollowBtn.Visible = false
    end

    stopFollowBtn.MouseButton1Click:Connect(StopLoopTp)

    decBtn.MouseButton1Click:Connect(function()
        followDist = math.max(0,followDist-1)
        distLabel.Text = "距离:"..followDist
    end)

    incBtn.MouseButton1Click:Connect(function()
        followDist = math.min(10,followDist+1)
        distLabel.Text = "距离:"..followDist
    end)
    ...
    local function Refresh()
        ...
        for _,p in pairs(Players:GetPlayers()) do
            if p == plr then continue end
            local item = Instance.new("Frame")
            item.Size = UDim2.new(0,75,0,75)
            item.BackgroundColor3 = Color3.new(0.18,0.18,0.22)
            item.Parent = scroll
            local corner = Instance.new("UICorner")
            ...
            avatar.Position = UDim2.new(0.5,-30,0.5,-30)
            ...
            item.MouseButton1Click:Connect(function()
                ...
                menu.Position = UDim2.new(0.5,-60,0.5,-45)
                ...
                btn1.MouseButton1Click:Connect(function()
                    pcall(function()
                        ...
                        if myRoot and tRoot then
                            myRoot.CFrame = tRoot.CFrame * CFrame.new(0,0,-followDist)
                        end
                    end)
                    menu:Destroy()
                end)
                ...
                btn2.Position = UDim2.new(0,0,0,30)
                ...
                btn3.Position = UDim2.new(0,0,0,60)
...
local function StartGraphicsOptimize()
    ...
    pcall(function()
        ...
        local groundY = playerRoot and (playerRoot.Position.Y - 5) or 2  -- 地面基准高度
        ...
        for _, obj in pairs(Workspace:GetDescendants()) do
            ...
            -- 处理实体部件
            if obj:IsA("BasePart") or obj:IsA("UnionOperation") or obj:IsA("NegateOperation") then
                -- 规则：
                -- 1. Y 坐标接近地面 → 视为地板，保留
                -- 2. 薄且平的物体（Size.Y < 1.2）→ 视为地面/地板，保留
                -- 3. 其他立体物体 → 删除
                local posY = obj.Position.Y
                local thickness = obj.Size.Y

                local isFloor = (posY <= groundY + 1.5) or (thickness < 1.2)
                if not isFloor then
                    -- 非地面 → 清除障碍物（隐藏并关闭碰撞）
                    obj.Transparency = 1
            ...
            if obj:IsA("Model") then
                ...
                if not hasHumanoid then
                    ...
                    if primary then
                        local posY = primary.Position.Y
                        local thickness = primary.Size.Y
                        local isFloorModel = (posY <= groundY + 1.5) or (thickness < 1.2)
                        if not isFloorModel then
                            obj:Destroy()
                        end
                    else
                        -- 没有 PrimaryPart 的模型，按名称粗略判断（可选）
                        local name = obj.Name:lower()
                        if not (name:find("floor") or name:find("ground") or name:find("terrain") or name:find("platform")) then
                            obj:Destroy()
                        end
                    end
                end
...
local function CreateCameraSwitchButton()
    pcall(function() if CoreGui:FindFirstChild("CameraSwitchBtn") then CoreGui.CameraSwitchBtn:Destroy() end end)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        if CameraState.IsFirstPerson then
            ...
            cam.CameraType = Enum.CameraType.Custom
            cam.CFrame = cam.CFrame * CFrame.new(0,0,-8)
        end
    end)
end
...
local function ShowCombatPage()
    ...
    CreateSwitchWithSetting("身体自动转向锁敌", function() return GlobalState.AutoAimTurn end, StartAutoAimTurn, StopAutoAimTurn, OpenAimTurnSettings, "AimTurnSettings")
    CreateSwitchWithSetting("玩家透视ESP", function() return ESPConfig.Enabled end, StartESP, StopESP, OpenESPSettings, "ESPSettings")
    CreateSwitchBtn("武器追踪", function() return WeaponTrackConfig.Enabled end, StartWeaponTrack, StopWeaponTrack)
end
...
local function ShowFunPage()
    ...
    CreateTitle("画质优化")
    CreateSwitchBtn("画质优化（流畅模式）", function() return GraphicsOptimizeConfig.Enabled end, StartGraphicsOptimize, StopGraphicsOptimize)
    -- 配置管理
    CreateTitle("配置管理")
