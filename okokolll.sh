...
-- AI全自动战斗核心（修复飞天+卡顿+指定单人+手动模式+技能服适配）
        ...
        pcall(function()
            ...
            if not target then
                for _, p in pairs(Players:GetPlayers()) do
                    ...
                    local tRoot = tChar and tChar:FindFirstChild("HumanoidRootPart")
                    local tHum = tChar and tChar:FindFirstChildOfClass("Humanoid")
                    if tRoot and tHum and tHum.Health > 0 and (root.Position - tRoot.Position).Magnitude <= AICombatConfig.LockRadius then
                        target = {Player = p, Root = tRoot, Hum = tHum}
                        break
                    end
                end
            end
            ...
            -- 寻路不飞天
            local path = PathfindingService:CreatePath({AgentCanJump = true, AgentHeight = 4})
            path:ComputeAsync(root.Position, target.Root.Position)
            local way = path:GetWaypoints()
            if way and #way > 0 then
                hum:MoveTo(way[2].Position)
            end

            -- 闪避
            if (root.Position - target.Root.Position).Magnitude < 8 then
                root.Velocity = Vector3.new(math.random(-1,1), 0, math.random(-1,1)).Unit * 25
            end

            -- 攻击/技能
            if not AICombatConfig.ManualAttack then
                local tool = char:FindFirstChildOfClass("Tool")
                if tool then tool:Activate() end
                for _, v in pairs(char:GetDescendants()) do
                    if v:IsA("BindableEvent") or v:IsA("RemoteEvent") and AICombatConfig.UseSkills then
...
local function OpenAICombatSettings()
    ...
    tarBtn.Position = UDim2.new(0.05,0,0,200)
    ...
    learnBtn.Position = UDim2.new(0.05,0,0,250)
    ...
    show.Position = UDim2.new(0.05,0,0,300)
