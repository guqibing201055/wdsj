...
local function StartClearBuildings()
    ...
    pcall(function()
        ...
        local groundY = playerRoot and (playerRoot.Position.Y - 3) or 0
        for _, obj in pairs(Workspace:GetDescendants()) do
            local isPlayer = false
            pcall(function() if plr.Character and obj:IsDescendantOf(plr.Character) then isPlayer = true end end)
            ...
            if (obj:IsA("BasePart") or obj:IsA("Model")) and not obj:FindFirstChildOfClass("Humanoid") then
                ...
                if part then
                    local isFloor = part.Size.Y < 1.5 or (part.Position.Y <= groundY + 2)
                    if not isFloor then
...
local function CreateCameraSwitchButton()
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        if CameraState.IsFirstPerson then
            ...
            cam.CameraType = Enum.CameraType.Custom
            cam.CameraSubject = hum
            hum.CameraOffset = Vector3.new(0,0,-8)
        end
    end)
end
