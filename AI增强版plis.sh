...
local AICombatConfig = {
    ...
    ManualAttack = false,
    UseSkills = true,
    Learned = {Move = false, Attack = false, Dodge = false, Skill = false},
    AIControlCamera = false   -- 【新增】视角控制开关
}
...
local AIEngine = {
    LastAttack = 0,
    LastDodge = 0,
    EnemyWeapon = {Type = "Melee", Range = 2, CD = 0.6, HasDamage = false},
    SelfWeapon = {Type = "Melee", Range = 18},
    PredictPos = nil
}
local TargetHintGui = nil
...
local function StartGraphicsOptimize()
    ...
    pcall(function()
        for _, v in pairs(Workspace:GetDescendants()) do
            ...
            if v:IsA("Fire") or v:IsA("Smoke") or v:IsA("ParticleEmitter") or v:IsA("PointLight") or v:IsA("SpotLight") or v:IsA("SurfaceLight") then
        ...
        while ClearBuildConfig.Enabled do
            ...
            for _, v in pairs(Workspace:GetDescendants()) do
                if (v:IsA("BasePart") or v:IsA("Model")) and not v:FindFirstChildOfClass("Humanoid") then
                    ...
                    local isFloor = p.Size.Y < 1.2 and p.Position.Y <= root.Position.Y + 1
                    if not isFloor then
                        if not ClearBuildConfig.OriginalState[v] then
                            ClearBuildConfig.OriginalState[v] = {
...
-- AI学习系统（引导步骤+勾选悬浮窗）
local function OpenAILearnWindow()
    local learn = {}
    local sg = Instance.new("ScreenGui")
    sg.Name = "AILearn"
    sg.Parent = CoreGui
    ...
    for i, o in pairs(opts) do
        ...
        b.Position = UDim2.new(0.05,0,0,30 + (i-1)*50)
        b.BackgroundColor3 = Color3.new(0.2,0.2,0.25)
        b.Text = o.Name
        b.TextColor3 = Color3.new(1,1,1)
        b.Parent = main
        b.MouseButton1Click:Connect(function()
            learn[o.Key] = not learn[o.Key]
            b.BackgroundColor3 = learn[o.Key] and Color3.new(0,0.6,0) or Color3.new(0.2,0.2,0.25)
        end)
    end
    local ok = Instance.new("TextButton")
    ok.Size = UDim2.new(0.9,0,0,40)
    ok.Position = UDim2.new(0.05,0,0,200)
    ok.BackgroundColor3 = Config.BtnGreen
    ok.Text = "开始学习"
    ...
    ok.MouseButton1Click:Connect(function()
        ...
        text.Text = "步骤1：移动角色到目标点 → 完成后点下一步"
        text.Parent = tip
        local next = Instance.new("TextButton")
        next.Size = UDim2.new(0,100,0,40)
        next.Position = UDim2.new(0.5,-50,0,50)
        next.BackgroundColor3 = Config.BtnBlue
        next.Text = "下一步"
        next.Parent = tip
        local step = 1
        next.MouseButton1Click:Connect(function()
            step = step + 1
            if step == 2 then
                text.Text = "步骤2：攻击敌人 → 完成后点下一步"
            elseif step == 3 then
                text.Text = "步骤3：闪避敌人攻击 → 完成后点下一步"
            elseif step == 4 then
                text.Text = "步骤4：使用技能 → 完成后点完成"
            elseif step == 5 then
                text.Text = "学习完成！"
                AICombatConfig.Learned = learn
                task.wait(1)
                tip:Destroy()
                next:Destroy()
            end
...
-- 终极AI战斗：敌我武器识别+4场景打法+视角控制+卡后摇反打
local function StartAICombat()
    AICombatConfig.Enabled = true
    pcall(function()
        if TargetHintGui then TargetHintGui:Destroy() end
        TargetHintGui = Instance.new("ScreenGui")
        TargetHintGui.Name = "TargetHint"
        TargetHintGui.Parent = CoreGui
        local hint = Instance.new("TextLabel")
        hint.Size = UDim2.new(0,220,0,32)
        hint.Position = UDim2.new(0.8,-110,0.9,-16)
        hint.BackgroundTransparency = 0.5
        hint.BackgroundColor3 = Color3.new(0,0,0)
        hint.TextColor3 = Color3.new(1,1,0)
        hint.TextScaled = true
        hint.Parent = TargetHintGui
    end)

    local aiLoop = RunService.Heartbeat:Connect(function()
        if not AICombatConfig.Enabled then
            if TargetHintGui then TargetHintGui:Destroy() end
            return
        end
        pcall(function()
            ...
            local target = AICombatConfig.Target
            if not target or not target.Character or target.Character.Humanoid.Health <=0 then
                target = nil
                local minD = AICombatConfig.LockRadius
                for _,p in pairs(Players:GetPlayers()) do
                    if p~=plr and p.Character and p.Character.Humanoid.Health>0 then
                        local d = (root.Position-p.Character.HumanoidRootPart.Position).Magnitude
                        if d < minD then minD=d; target=p end
                    end
                end
            end
            if TargetHintGui then
                TargetHintGui.TextLabel.Text = target and target.Name.." 被AI追踪" or "未锁定目标"
            end
            ...
            AIEngine.EnemyWeapon.Type = tTool and "Ranged" or "Melee"
            AIEngine.EnemyWeapon.Range = AIEngine.EnemyWeapon.Type=="Ranged" and 16 or 2
            AIEngine.EnemyWeapon.HasDamage = false

            for _,track in pairs(tHum:GetPlayingAnimationTracks()) do
                local n = track.Name:lower()
                if n:find("attack") or n:find("swing") or n:find("shoot") then
                    AIEngine.EnemyWeapon.HasDamage = true
                    break
                end
            end

            local selfTool = char:FindFirstChildOfClass("Tool")
            AIEngine.SelfWeapon.Type = selfTool and "Ranged" or "Melee"
            AIEngine.SelfWeapon.Range = AIEngine.SelfWeapon.Type=="Ranged" and 18 or 2.1

            if AICombatConfig.AIControlCamera then
                local aimPos = Vector3.new(tRoot.Position.X, root.Position.Y+1.2, tRoot.Position.Z)
                cam.CFrame = CFrame.new(cam.CFrame.Position, aimPos)
            end

            if AIEngine.SelfWeapon.Type == "Melee" and AIEngine.EnemyWeapon.Type == "Melee" then
                local safe = 2.1
                if AIEngine.EnemyWeapon.HasDamage then
                    if tick()-AIEngine.LastDodge>0.3 then
                        local dir = (root.Position-tRoot.Position).Unit
                        hum:MoveTo(root.Position+dir*3)
                        AIEngine.LastDodge = tick()
                    end
                else
                    if dist > safe+0.2 then
                        hum.WalkSpeed=18; hum:MoveTo(tRoot.Position)
                    elseif dist < safe-0.2 then
                        hum.WalkSpeed=16; hum:MoveTo(root.Position+(root.Position-tRoot.Position).Unit*1)
                    else
                        hum.WalkSpeed=0
                        if selfTool and tick()-AIEngine.LastAttack>0.5 then
                            selfTool:Activate()
                            AIEngine.LastAttack = tick()
                        end
                    end
                end
            elseif AIEngine.SelfWeapon.Type == "Ranged" and AIEngine.EnemyWeapon.Type == "Melee" then
                local keepDist = 16
                if dist > keepDist+2 then
                    hum.WalkSpeed=16; hum:MoveTo(tRoot.Position)
                elseif dist < keepDist-2 then
                    hum.WalkSpeed=16; hum:MoveTo(root.Position+(root.Position-tRoot.Position).Unit*5)
                else
                    hum.WalkSpeed=0
                    if selfTool and tick()-AIEngine.LastAttack>0.4 then
                        selfTool:Activate()
                        AIEngine.LastAttack = tick()
                    end
                end
            elseif AIEngine.SelfWeapon.Type == "Melee" and AIEngine.EnemyWeapon.Type == "Ranged" then
                if dist > 5 then
                    hum.WalkSpeed=20
                    local movePos = tRoot.Position + Vector3.new(math.random(-3,3),0,math.random(-3,3))
                    hum:MoveTo(movePos)
                else
                    if AIEngine.EnemyWeapon.HasDamage then
                        local dir = Vector3.new(math.random(-1,1),0,math.random(-1,1)).Unit*4
            ...
            elseif AIEngine.SelfWeapon.Type == "Ranged" and AIEngine.EnemyWeapon.Type == "Ranged" then
                local combatDist = 14
                if dist > combatDist+3 then
                    hum.WalkSpeed=16; hum:MoveTo(tRoot.Position)
                elseif dist < combatDist-3 then
                    hum.WalkSpeed=16; hum:MoveTo(root.Position+(root.Position-tRoot.Position).Unit*3)
                else
                    hum.WalkSpeed=12
                    local sidePos = root.Position + Vector3.new(math.random(-2,2),0,math.random(-2,2))
                    hum:MoveTo(sidePos)
                    if selfTool and tick()-AIEngine.LastAttack>0.35 then
                        selfTool:Activate()
                        AIEngine.LastAttack = tick()
                    end
                end
    ...
    table.insert(_G.GlobalFunLoops, aiLoop)
end
...
local function OpenAICombatSettings()
    ...
    local function Num(y,t,k)
        ...
        i.Position = UDim2.new(0.5,0,0,2)
        i.BackgroundColor3 = Config.InputBg
        i.Text = tostring(AICombatConfig[k])
        i.Parent = f
        local b = Instance.new("TextButton")
        b.Size = UDim2.new(0.18,0,0,35)
        b.Position = UDim2.new(0.82,0,0,2)
        ...
        b.MouseButton1Click:Connect(function()
            local v = tonumber(i.Text)
            if v then AICombatConfig[k] = math.clamp(v,10,100) end
        end)
    end
    Num(50, "锁定半径", "LockRadius")
    -- 手动攻击
    local function Toggle(y,t,k)
        ...
        b.Size = UDim2.new(0.3,0,0,35)
        b.Position = UDim2.new(0.7,0,0,2)
        b.BackgroundColor3 = AICombatConfig[k] and Color3.new(0,0.6,0) or Color3.new(0.3,0.3,0.3)
        b.Text = AICombatConfig[k] and "开启" or "关闭"
        b.Parent = f
        b.MouseButton1Click:Connect(function()
            AICombatConfig[k] = not AICombatConfig[k]
        end)
    end
    Toggle(100, "手动攻击", "ManualAttack")
    Toggle(150, "自动技能", "UseSkills")
    Toggle(200, "AI视角控制", "AIControlCamera")   -- 【新增】
    -- 指定目标
    local tarBtn = Instance.new("TextButton")
    tarBtn.Size = UDim2.new(0.9,0,0,40)
    tarBtn.Position = UDim2.new(0.05,0,0,250)
    tarBtn.BackgroundColor3 = Config.BtnPurple
    tarBtn.Text = "指定攻击目标"
    tarBtn.Parent = main
    tarBtn.MouseButton1Click:Connect(function()
        OpenPlayerLockWindow()
    end)
    -- AI学习
    local learnBtn = Instance.new("TextButton")
    learnBtn.Size = UDim2.new(0.9,0,0,40)
    learnBtn.Position = UDim2.new(0.05,0,0,300)
    learnBtn.BackgroundColor3 = Config.BtnOrange
    learnBtn.Text = "AI学习"
    learnBtn.Parent = main
    learnBtn.MouseButton1Click:Connect(OpenAILearnWindow)
    -- 已学习内容
    local show = Instance.new("TextLabel")
    show.Size = UDim2.new(0.9,0,0,30)
    show.Position = UDim2.new(0.05,0,0,350)
    show.BackgroundTransparency = 1
    show.Text = "已学习: "..(AICombatConfig.Learned.Move and "移动 " or "")..(AICombatConfig.Learned.Attack and "攻击 " or "")
